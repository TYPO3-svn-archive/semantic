...
namespace T3\Semantic\Cache\Frontend;
/***************************************************************
 *  Copyright notice
 *
...
class QueryCache {
	 ...
	 * @access		public
	 * @param		\T3\Semantic\Cache\Backend\QueryCache\Backend			$backend	backend implementation
	 */
	 ...
	 * @access		public
	 * @returns	\T3\Semantic\Cache\Backend\QueryCache\Backend			$backend	backend implementation
	 */
	...
	public function save($queryString, $resultFormat = "plain", $queryResult, $duration = 0) {
		...
		if (!($this->backend instanceof \T3\Semantic\Cache\Backend\QueryCache\Null)) {
			...
			if (!($objectCache->getBackend() instanceof \T3\Semantic\Cache\Backend\Null)) {
	...
	public function getMaterializedViewName($subject, $predicate, $object) {
		...
		if (!($subject instanceof \T3\Semantic\Rdf\Resource)) {
			$subject = null;
		} else {
			$subject = (string)$subject;
		}

		if (!($predicate instanceof \T3\Semantic\Rdf\Resource)) {
			$predicate = null;
		} else {
			$predicate = (string)$predicate;
		}

		if (!($object instanceof \T3\Semantic\Rdf\Resource)) {
			$object = null;
		} else {
