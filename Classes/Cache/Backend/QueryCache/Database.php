...
namespace T3\Semantic\Cache\Backend\QueryCache;
/***************************************************************
 *  Copyright notice
 *
	...
	private function _query($sql, $limit = PHP_INT_MAX, $offset = 0) {
		...
		try {
			...
			if (!$this->checkCacheVersion()) {
				...
				catch (\T3\Semantic\Store\Adapter\Exception $e2) {
				...
				throw new \T3\Semantic\Exception('Something went wrong with the query cache: ' . $e->getMessage() . ' SQL:' . $sql);
			}
		}
