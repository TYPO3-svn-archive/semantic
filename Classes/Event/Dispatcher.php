 ...
 */
class Dispatcher implements \t3lib_Singleton {
		...
		if (is_object($handler)) {
			...
			if (is_array($handler)) {
				...
				if (!array_key_exists('class_name', $handler)) {
					throw new \T3\Semantic\Exception("Missing key 'class_name' for handler registration.");
				}

				if (!array_key_exists('include_path', $handler)) {
					throw new \T3\Semantic\Exception("Missing key 'include_path' for handler registration.");
				}
