...
namespace T3\Semantic\Syntax\RdfParser\Adapter;
/***************************************************************
 *  Copyright notice
 *
...
class Turtle implements AdapterInterface {
	...
	public function parseFromFilename($filename) {
		...
		if ($fileHandle === false) {
			...
			throw new \T3\Semantic\Syntax\RdfParserException("Failed to open file with filename '$filename'");
		}
	...
	protected function _addNamespacesToStore() {
		...
		foreach ($this->_namespaces as $prefix => $ns) {
			...
			catch (\T3\Semantic\Namespaces\Exception $e) {
	...
	protected function _addNamespace($prefix, $ns) {
		...
		if ($ns instanceof \T3\Semantic\Rdf\Resource) {
			$ns = $ns->getUri();
		} else {
			$ns = (string)$ns;
		}
	...
	protected function _parseUri() {
		...
		return \T3\Semantic\Rdf\Resource::initWithIri($uri);
	}
	...
	protected function _throwException($msg) {
		throw new \T3\Semantic\Syntax\RdfParserException($msg);
	}
	...
	protected function _parseNumber() {
		...
		return \T3\Semantic\Rdf\Literal::initWithLabelAndDatatype($token, $datatype);
	}
	...
	protected function _parseQuotedLiteral() {
		...
		if ($c === '@') {
			...
			if (!$this->_isLanguageStartChar($c)) {
				...
				throw new \T3\Semantic\Syntax\RdfParserException('Character "' . $c . '" not allowed as starting char in language tags.');
			}
			...
			while ($c !== -1 && !$this->_isWS($c) && !($c === ',') && !($c === '.')) {
				if (!$this->_isLanguageChar($c)) {
					...
					throw new \T3\Semantic\Syntax\RdfParserException('Character "' . $c . '" not allowed in language tags.');
				}
			...
			$this->_unread();
			return \T3\Semantic\Rdf\Literal::initWithLabelAndLanguage($label, $lang);
		} else {
			if ($c === '^') {
				$this->_read();
				...
				if ($datatype instanceof \T3\Semantic\Rdf\Resource) {
					return \T3\Semantic\Rdf\Literal::initWithLabelAndDatatype($label, (string)$datatype);
				} else {
					$this->_throwException('Illegal datatype value.');
					return null;
				}
			} else {
				return \T3\Semantic\Rdf\Literal::initWithLabel($label);
			}
		}
	}
	...
	protected function _parseQNameOrBoolean() {
		...
		if ($c === ':') {
			...
			if ($c !== ':') {
				...
				if ($prefix === 'true' || $prefix === 'false') {
					return \T3\Semantic\Rdf\Literal::initWithLabelAndDatatype($value, EF_XSD_BOOLEAN);
				}
			}
		...
		return \T3\Semantic\Rdf\Resource::initWithNamespaceAndLocalName($namespace, $localName);
	}
	...
	protected function _createBNode($id = null) {
		...
		return \T3\Semantic\Rdf\Resource::initWithBlankNode($id);
	}
	...
	protected function _parsePredicate() {
		...
		if ($predicate instanceof \T3\Semantic\Rdf\Resource && !$predicate->isBlankNode()) {
			return $predicate;
		} else {
	...
	protected function _addStatement($s, $p, $o) {
		if ($s instanceof \T3\Semantic\Rdf\Resource) {
			if ($s->isBlankNode()) {
				$s = '_:' . $s->getId();
			} else {
				$s = $s->getUri();
		...
		if ($p instanceof \T3\Semantic\Rdf\Resource) {
			$p = $p->getUri();
		}

		if ($o instanceof \T3\Semantic\Rdf\Resource) {
			if ($o->isBlankNode()) {
				$o = '_:' . $o->getId();
				$oType = 'bnode';
			} else {
			...
			if ($o instanceof \T3\Semantic\Rdf\Literal) {
				$dType = $o->getDatatype();
				$lang = $o->getLanguage();
				$o = $o->getLabel();
				$oType = 'literal';
