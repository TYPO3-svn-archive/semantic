...
class Turtle implements AdapterInterface {
	...
	public function serializeQueryResultToString($query, $graphUri, $pretty = false, $useAc = true) {
		...
		$s = $this->objectManager->create('\T3\Semantic\Sparql\Query2\Variable', 'resourceUri');
		$p = $this->objectManager->create('\T3\Semantic\Sparql\Query2\Variable', 'p');
		$o = $this->objectManager->create('\T3\Semantic\Sparql\Query2\Variable', 'o');
		if (strstr((string)$query, '?resourceUri ?p ?o') === false) {
			if ($query instanceof \T3\Semantic\Sparql\Query2) {
				$query->addTriple($s, $p, $o);
			} else {
				//should not happen
				throw new \Exception('serializeQueryResultToString expects a \T3\Semantic\Sparql\Query2 object');
			}
		}
		...
		if ($query instanceof \T3\Semantic\Sparql\Query2) {
			...
			if ($query instanceof \T3\Semantic\Sparql\SimpleQuery) {
		...
		if ($oType === 'uri') {
			...
			if ($oType === 'bnode') {
				...
				if (strpos($o, "\n") !== false || strpos($o, "\r") !== false || strpos($o, "\t") !== false) {
					$this->write('"""');
					$this->write(\T3\Semantic\Syntax\Utils\Turtle::encodeLongString($o));
					$this->write('"""');
				} else {
					$this->write('"');
					$this->write(\T3\Semantic\Syntax\Utils\Turtle::encodeString($o));
					$this->write('"');
				}
	...
	protected function _writeUri($uri) {
		...
		$splitIdx = \T3\Semantic\Syntax\Utils\Turtle::findUriSplitIndex($uri);
		if ($splitIdx !== false) {
	...
	protected function throwException($msg) {
		throw new \T3\Semantic\Syntax\RdfSerializerException($msg);
	}
