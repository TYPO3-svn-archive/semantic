...
namespace T3\Semantic\Sparql\EngineDb\Adapter;
/***************************************************************
 *  Copyright notice
 *
...
use \T3\Semantic\Sparql\EngineDb;
use \T3\Semantic\Sparql\EngineDb\ResultRenderer;
use \T3\Semantic\Sparql\EngineDb\SqlGenerator\Adapter;
class Typo3 {
	...
	public function queryModel(Sparql\Query $query, $resultform = 'plain') {
		$this->query = $query;
		$qsimp = $this->objectManager->create('\T3\Semantic\Sparql\EngineDb\QuerySimplifier');
		$qsimp->simplify($this->query);
		$queryOptimizer = $this->objectManager->create('\T3\Semantic\Sparql\EngineDb\QueryOptimizer', $this);
		$result = $queryOptimizer->optimize($this->query);
		if ($result instanceof \T3\Semantic\Sparql\Query) {
			$this->query = $result;
		}
		$resultform = strtolower($resultform);
		switch ($resultform) {
			case 'xml':
				$rc = $this->objectManager->create('\T3\Semantic\Sparql\EngineDb\ResultRenderer\Xml');
				break;
			//throw new Erfurt_Exception('XML result format not supported yet.');
			//$this->rc = new ResultRenderer\RapZendDb_Xml();
			//break;
			case 'extended':
				$rc = $this->objectManager->create('\T3\Semantic\Sparql\EngineDb\ResultRenderer\Extended');
				break;
			case 'json':
				$rc = $this->objectManager->create('\T3\Semantic\Sparql\EngineDb\ResultRenderer\Json');
				break;
			case 'plain':
			default:
				$rc = $this->objectManager->create('\T3\Semantic\Sparql\EngineDb\ResultRenderer\Plain');
		}
		if (is_array($result)) {
			$result = $rc->convertFromDbResults($result['data'], $this->query, $this, $result['vars']);
			return $result;
		}
		$this->sg = $this->objectManager->create('\T3\Semantic\Sparql\EngineDb\SqlGenerator\Adapter\Typo3', $this->query, $this->arModelIdMapping);
		$this->ts = $this->objectManager->create('\T3\Semantic\Sparql\EngineDb\TypeSorter', $this->query, $this);
		$this->_setOptions();
