...
class Parser {
		...
		switch (substr(current($this->_tokens), 0, 1)) {
			case '^':
				if (substr(current($this->_tokens), 0, 2) === '^^') {
					...
					$node = Rdf\Literal::initWithLabel(substr($node, 1, -1));
					$node->setDatatype(
				...
				break;
			case '@':
				$node = Rdf\Literal::initWithLabelAndLanguage(
			...
			default:
				prev($this->_tokens);
				$node = Rdf\Literal::initWithLabel(substr($node, $nSubstrLength, -$nSubstrLength));
				break;
		}
	}
	...
	protected function _dtypeCheck(&$node) {
		...
		if ($match > 0) {
			$node = Rdf\Literal::initWithLabel($hits[0]);
	...
	protected function _parseBase() {
		...
		if ($this->_iriCheck(current($this->_tokens))) {
			...
			throw new ParserException('IRI expected', -1, key($this->_tokens));
		}
	}
	...
	protected function _parseGraph() {
		...
		if ($this->_iriCheck($name)) {
			...
			if ($this->_qnameCheck($name)) {
				$name = Rdf\Resource::initWithIri($this->_query->getFullUri($name));
			}
		}
			...
			switch (strtolower(current($this->_tokens))) {
				case 'order':
					...
					if (strtolower(current($this->_tokens)) === 'by') {
						...
						throw new ParserException('"BY" expected.', -1, key($this->_tokens));
					}
					break;
				case 'limit':
					$this->_fastForward();
	...
	protected function _parseNode($node = false) {
		...
		if ($this->_iriCheck($node)) {
			...
			if ($base != null) {
				$node = Rdf\Resource::initWithNamespaceAndLocalName(substr($base, 1, -1), substr($node, 1, -1));
			} else {
				$node = Rdf\Resource::initWithIri(substr($node, 1, -1));
			}
			return $node;
		} else {
			if ($this->_qnameCheck($node)) {
				$node = $this->_query->getFullUri($node);
				$node = Rdf\Resource::initWithIri($node);
				return $node;
			} else {
					...
					if ($this->_varCheck($node)) {
						...
						if ($node[0] === '<') {
							...
							if (substr($node, -1) != '>') {
								...
								exit;
								throw new ParserException('Unclosed IRI: ' . $node, -1, key($this->_tokens));
							}
