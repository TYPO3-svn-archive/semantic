...
namespace T3\Semantic\Authentication\Adapter;
/***************************************************************
 *  Copyright notice
 *
...
class Typo3 implements AdapterInterface {
	...
	public function authenticate() {
		if ($this->isLoginDisabled() === true || $this->username === 'Anonymous' || (TYPO3_MODE == 'FE' && !$GLOBALS["TSFE"]->loginUser && !isset($GLOBALS['BE_USER']))) {
			...
			$authResult = new \T3\Semantic\Authentication\Result(\T3\Semantic\Authentication\Result::FAILURE, null, array('No user either Frontend or Backend given.'));
		}
	...
	protected function getAnonymousUser() {
		...
		$identityObject = $this->objectManager->create('\T3\Semantic\Authentication\Identity', $user);
