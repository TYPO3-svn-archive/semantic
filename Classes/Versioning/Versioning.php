...
namespace T3\Semantic\Versioning;
/***************************************************************
 *  Copyright notice
 *
...
class Versioning {
	...
	/**
	 * Constructor registers with \T3\Semantic\Event\Dispatcher
	...
	public function onAddStatement(\T3\Semantic\Event\Event $event) {
		$this->checkSetup();
	...
	public function onAddMultipleStatements(\T3\Semantic\Event\Event $event) {
		$this->checkSetup();
	...
	public function onDeleteMatchingStatements(\T3\Semantic\Event\Event $event) {
		$this->checkSetup();
	...
	public function onDeleteMultipleStatements(\T3\Semantic\Event\Event $event) {
		$this->checkSetup();
	...
	public function startAction($actionSpec) {
		...
		if (null !== $this->_currentAction) {
			...
			$this->_currentActionParent = $this->execAddAction($graphUri, $resource, $actionType);
		} else {
			// do nothing
		}
	}
	...
	private function execAddAction($graphUri, $resource, $actionType, $payloadId = null) {
		if ($this->_user === null) {
			$this->_user = $this->getAuthentication()->getIdentity()->getUri();
		}
		$userUri = $this->_user;
	...
	protected function sqlQuery($sql, $limit = PHP_INT_MAX, $offset = 0) {
		try {
			...
			catch (\T3\Semantic\Exception $e2) {
				throw new \T3\Semantic\Exception('Erfurt_Versioning _sqlQuery failed: ' . $e2->getMessage() . $sql);
			}
		}
