...
namespace T3\Semantic\ExtensionManager;
/***************************************************************
* Copyright notice
*
...
class ConstantsExtended {
	...
	function textarea($params, $pObj) {
		...
		$field = '<textarea '.$css.' id="field'.$key.'" cols="79" rows="5" wrap="hard" name="'. $fieldName .'">'. $fieldValue .'</textarea>';
	...
	function getConf($fieldName, $pObj) {
		...
		$conf = \t3lib_div::trimExplode(';', $pObj->flatSetup[$key.'..']);
		foreach ($conf as $key=>$value) {
